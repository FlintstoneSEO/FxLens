...
export const solutionReviewResponseSchema = z.object({
  area: z.literal("solution_review"),
  generatedAt: z.string().min(1),
  inventorySummary: z.string().min(1),
  architecturalFindings: z.array(z.string().min(1)),
  riskLevel: severitySchema,
  recommendedRefactors: z.array(z.string().min(1)),
  performanceRecommendations: z.array(performanceRecommendationSchema)
});
