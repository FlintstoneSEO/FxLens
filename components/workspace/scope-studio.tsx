...
export function ScopeStudio() {
  const [formState, setFormState] = React.useState<ScopeFormState>(initialFormState);
  const [result, setResult] = React.useState<ScopeResponse | null>(null);
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col gap-3 rounded-xl border border-border/70 bg-background/60 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3 text-sm text-muted-foreground">
              ...
              <div>
                <p className="font-medium text-foreground">{isSubmitting ? "Generating scope draft..." : "Ready to generate"}</p>
                <p>
                  {isSubmitting
          ...
          {result ? (
            ...
            <div className="rounded-xl border border-dashed border-border bg-background/50 p-6 text-sm text-muted-foreground">
              <div className="flex items-start gap-3">
                ...
                <p>Submit the form to preview a scope draft with modules, entities, automations, and implementation guidance.</p>
              </div>
            </div>
          )}
        </SectionCard>
